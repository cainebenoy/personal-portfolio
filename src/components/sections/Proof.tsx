    ...
    <section id="proof" aria-label="Proof — running totals" className="px-page py-24 sm:py-32">
      <div className="mx-auto max-w-7xl">
        <div className="flex items-center gap-5">
          ...
          <span className="kicker text-ivory/50">Proof</span>
        </div>
        ...
        >
          {STATS.map((stat) => (
            ...
            >
              <dd
                data-counter
                className="order-1 font-display text-[clamp(2.4rem,4vw,3.4rem)] leading-none text-ivory"
              >
                {stat.value}
              </dd>
              <dt className="mono-tag order-2 text-ivory/50">{stat.label}</dt>
            </div>
          ))}
        </dl>
      </div>
